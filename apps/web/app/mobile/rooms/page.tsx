  ...
  return (
    <div className="flex flex-col gap-5">
      <header>
        ...
        <p className="mt-1 text-sm leading-6 text-text-primary/80">{t.t('mobile.rooms.subtitle')}</p>
      </header>

      {/* Auth + Supabase load only on this route, mirroring `/play/online`. */}
      <AuthContextProvider>
        <Suspense fallback={<p className="text-sm text-text-primary/70">{t.t('rooms.creating')}</p>}>

-- Add Jhabbu as a third online game key. Mirrors migration 0008: rooms and
-- games may now host 'jhabbu' alongside 'gadha_chor' and 'lal_satti', and
-- create_room() accepts it. The server-authoritative room/persistence boundary
-- is unchanged — only the allowed set of game keys widens.

alter table public.rooms
  drop constraint if exists rooms_game_key_check;
alter table public.rooms
  add constraint rooms_game_key_check
    check (game_key in ('gadha_chor', 'lal_satti', 'jhabbu'));
...
-- otherwise identical to migration 0008.
...
begin
  ...
  if p_game_key not in ('gadha_chor', 'lal_satti', 'jhabbu') then

...
import type { JhabbuState } from './types';

/**
 * Fair play: a Jhabbu bot follows suit and sheds from its own hand and the
 * public trick only. It must never react to which hidden cards its opponents
 * hold. We replay real bot-vs-bot rounds and, at every turn, scramble the
 * identities of every non-acting hand (keeping each opponent's card count —
 * which is public — and the trick, waste, and led suit untouched), then assert
 * the bot makes the exact same move.
 */
...
function scrambleHiddenHands(state: JhabbuState, actorIndex: number, rng: Rng): JhabbuState {
...
function hiddenHandsChanged(before: JhabbuState, after: JhabbuState, actorIndex: number): boolean {
...
/** Deterministic `hard` bots in Jhabbu can legitimately cycle (both players
 * draw-and-shed to dodge the Thulla), so — like the engine's own bounded sweep
 * — we assert the per-turn invariants up to a cap and stop without demanding a
 * completed round. */
function runFairPlayRound(playerCount: number, seed: number): FairPlayRun {
  ...
  for (let turn = 0; turn < 600; turn += 1) {
    if (state.phase === 'round_complete') return { turns: turn, scrambleObserved };
    ...
    const real = chooseJhabbuBotAction(state, actor);
    ...
    const shadow = chooseJhabbuBotAction(scrambled, actor);
    expect(shadow?.action).toEqual(real!.action);

    state = engine.reduce(state, real!.action).state;
  }

  return { turns: 600, scrambleObserved };
}

describe('Jhabbu fair play', () => {
  it.each([3, 4, 5, 6])(
    'a %i-player bot round never lets a decision depend on hidden hands',
    (playerCount) => {
      const run = runFairPlayRound(playerCount, 909 + playerCount);
  ...
  it('holds across many seeds without an illegal or hidden-info-dependent move', () => {
    ...
    for (let seed = 1; seed <= 40; seed += 1) {
      const run = runFairPlayRound(4, seed);
